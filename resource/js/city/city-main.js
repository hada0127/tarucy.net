...
/**
 * 창문과 유사한 색상인지 확인
 * 실제 GLB에서 발견된 색상 (콘솔 로그 기준):
 * - 핑크: #be1e3f(190,30,63), #ff1e47(255,30,71), #ce1447(206,20,71),
 *         #de1430(222,20,48), #ff1437(255,20,55), #ff2950(255,41,80)
 * - 시안: #1e93af(30,147,175), #14a1be(20,161,190)
 */
function isWindowLikeColor(r, g, b) {
  // 완전 흰색이나 검정은 제외
  if ((r > 250 && g > 250 && b > 250) || (r < 20 && g < 20 && b < 20)) {
    return false;
  }

  // 핑크/빨강 계열 (실제 GLB 색상 기준)
  // R > 180, G < 50, B: 40~100
  const isPink = r > 180 && g < 50 && b >= 40 && b <= 100;

  // 시안 계열 (실제 GLB 색상 기준)
  // R < 50, G > 140, B > 160
  const isCyan = r < 50 && g > 140 && b > 160;
...
export async function initCity() {
  ...
  if (USE_GLB) {
    ...
    try {
      ...
      createAllBuildings(dummyScene, true);
      ...
      console.log(`Windows: ${getWindowDataList().length}`);
